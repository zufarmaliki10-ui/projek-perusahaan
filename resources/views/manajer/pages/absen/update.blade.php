...
<div class="p-3">
    ...
    <div class="col-md-12 my-sm-1 px-sm-1 col-xl-12 my-xl-0 px-xl-3">
        <a class="btn btn-primary btn-sm bi bi-arrow-left mb-2" href="{{route('manajer.absensi')}}"><span class="mx-1">Kembali</span></a>
        <div class="card p-2 mx-auto my-3">
            <h4>Form Validasi Absen Karyawan</h4>
            @if ($absensi->keterangan && $absensi->jam_keluar)
            <form action="{{route('manajer.absensi.update', $absensi->id)}}" method="post">
                @csrf
                @method('PUT')
